...
export function TrafficAggregatorWidget({ data }: { data: any[] }) {
  ...
  return (
    <div className="bg-[#09090b] rounded-lg border border-[#27272a] shadow-lg flex flex-col hover:border-[rgba(43,127,255,0.3)] transition-colors">
      ...
      <div 
        className="flex items-center justify-between px-4 py-3 border-b border-[#27272a] cursor-pointer group" 
        ...
        {data.map((ag) => {
          return (
            <div 
              key={ag.id} 
              onClick={() => navigate('/drilldown/aggregator')} 
              className="border border-[#27272a] bg-[rgba(24,24,27,0.4)] rounded-lg p-4 flex flex-col items-center justify-center gap-3 hover:border-[#2b7fff] cursor-pointer hover:bg-[rgba(43,127,255,0.05)] transition-all"
            >
